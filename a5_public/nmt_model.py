...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        source_padded = self.vocab.src.to_input_tensor_char(source, device=self.device)  # Tensor: (src_len, b)
        target_padded = self.vocab.tgt.to_input_tensor_char(target, device=self.device)   # Tensor: (tgt_len, b)
        source_padded_chars = self.model_embeddings_source(source_padded)
        target_padded_chars = self.model_embeddings_target(target_padded)
        ...
        target_gold_words_log_prob = torch.gather(P, index=target_padded[1:], dim=-1).squeeze(-1) * target_masks[1:]
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens, lens_hidden = pad_packed_sequence(enc_hiddens)
