...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        if len(unfinished_parses)>=batch_size:
            ...
            minibatch = unfinished_parses[:len(unfinished_parses)]
            current_batch = len(unfinished_parses)
        #print([obj.buffer for obj in minibatch])
        transitions = model.predict(minibatch)
        for i, pp in enumerate(minibatch):
            pp.parse_step(transitions[i])

        unfinished_parses = [parse_obj for parse_obj in unfinished_parses if not (parse_obj.buffer==[] and len(parse_obj.stack)==1)]
